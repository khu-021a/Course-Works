...
local out_dir = pl.path.join(base_dir, 'out')

local input_path = pl.path.join(data_dir, 'test_inputs.t7')
local target_path = pl.path.join(data_dir, 'test_targets.t7')
local out_model_path = pl.path.join(out_dir, 'out_model.t7')
...
if not train then
	srcnn_model:clearState()
	torch.save(out_model_path, srcnn_model)
end
